...
| 层级 | 名称 | 目录 | 核心目的 |
|------|------|------|----------|
| L0 | 战略与愿景 | `L0-vision/` | 定义项目"为什么"，商业目标、成功标准与战略对齐 |
...
| L4 | 逻辑/系统设计 | `L4-system-design/` | 内部结构、模块职责、接口契约、动作、领域模型 |
| L5 | 详细设计 | `L5-detail-design/` | 算法、流程与约束；组件引用 L3 CMP，不在本层重复定义 |
| L6 | 验证与确认 | `L6-verification/` | 测试策略、测试场景、需求追溯（非代码） |
| REF | 参考与引用 | `references/` | 外部资料来源（文档、链接），供所有层级引用 |
| ADR | 架构决策记录 | `L3-architecture/`（与层级文档平放，无独立目录） | 宏观架构级决策的一事一档，见 [type-profiles.md · ADR 与 DEC 的定位](type-profiles.md#adr-与-dec-的定位) |
...
- **补齐信号**：出现愿景、商业目标、成功指标、路线图主题 → L0；出现角色分层、利益相关者诉求、业务场景冲突 → L1；出现明确功能需求、非功能约束、验收口径 → L2。未命中则先不建对应层；命中后再补建并建立追溯引用。
