...
- **细项编码**：**编码形式**默认使用简洁编码（`{类型码}-{三位序号}`）；**项目前缀**仅在 `ued/.doc-config.json` 已配置 `project_code` 且用户明确确认启用时添加（细则见 `references/coding-system.md`）。正文中定义的细项须在文末维护**本文档细项编码清单**；**清单范围**仅本文档，**分配流程**为分配前检索 `ued/README.md` 全局索引、分配后同步更新。
...
2. **复制内容**：以模板为起点编写
3. **替换占位符**：更新 `{编号}`、`{文档名称}` 等；仅在扩展模式启用且用户确认时使用 `{项目编码}` 前缀（见 coding-system）
4. **设置状态**：从标准状态值中选择
5. **维护编码**：全局唯一；文档末尾维护“本文档细项编码清单”（仅本文档范围），并同步更新 `ued/README.md` 全局索引
