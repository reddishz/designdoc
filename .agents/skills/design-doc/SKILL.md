...
**触发时机**：AI 每完成一轮 `ued/` 下文档的实质修改后，收尾时**主动**执行以下流程。此为"状态即基线"的操作化闭环——给未发布的 `草稿` 一个自然晋升窗口，同时避免"AI 自做主张升格"。

**执行步骤（MUST COMPLY）**：

1. **提交提示**：向用户询问"本轮修改已完成，是否需要提交到版本管理系统（git / svn）？"。用户若答"暂不提交"，跳过 2–4，所有 `草稿` 保持不变。
2. **草稿扫描**：用户确认要提交时，**仅扫描本轮 AI 实际修改过的文件**，收集其中仍为 `草稿` 的对象：
   - **文档级**：元信息表 `状态 = 草稿` 的整份文档；
   - **细项级**：正文带 `**细项状态**：草稿`，或索引/清单表状态列为 `草稿` 的编码。
3. **呈现与确认**：以紧凑表格一次性列出，请求用户逐项或批量确认。支持"全定稿 / 全保留 / 除 X,Y 外全定稿 / 逐项指定"等批量口径。
4. **执行升格与提交**：
   - 用户**明确确认**的对象 → 状态改为 `正式`；版本号按约定确立或沿用（首次定稿取 `v1.0`；已是正式对象在修订轮次中定稿时，沿用本轮已提升的版本号）。
   - 用户**未提及 / 略过 / 拒绝**的对象 → **保留 `草稿`**，不改动状态或版本。
   - 升格完成后，按用户指示执行 `svn commit` / `git push`，或交回用户自行提交。

**约束（MUST NOT）**：
- AI **MUST NOT** 未经用户明确确认擅自把任何 `草稿` 改为 `正式`。
- AI **MUST NOT** 扫描并提交本轮**未修改**的历史草稿对象——那属于 `check_docs.py` 或人工审查的静态提示范畴。
- 未提及 = 未确认 = 保留草稿。
