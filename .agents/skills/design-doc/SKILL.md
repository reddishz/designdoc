...
分配步骤见上文；缺口与计数器等式见 [object-model.md · 编码含义锁定](references/object-model.md#编码含义锁定mandatory)；索引操作见 [coding-system.md · 编码分配管理](references/coding-system.md#编码分配管理)。

### 处理编码含义变更请求

响应触发（三类）：（a）改已分配编码的 ID 或类型码——**不区分状态**；（b）改已 `正式` / `草案` 对象的**标题**；（c）改已 `正式` 编码的业务含义。`初稿` 改标题（须同步引用）与 `初稿` / `草案` 改正文不属此列。通道见 [object-model.md · 修改操作矩阵](references/object-model.md#修改操作矩阵must-comply)。AI **MUST** 按以下方式响应：
...
建议处理方式：
1. （初稿）同步全部引用处后修改标题，或作废该细项后按分配流程以正确类型码重建；（正式 / 草案）废弃原编码 [{编码}]（原含义：{原标题}）
2. 分配新编码承载新标题 / 新含义
...
**Checkpoint 4**：标注 `废弃` 前 **MUST** 暂停并确认原因与替代方案。`草案` 可直接废弃（最后版本号即该草案号）；`初稿→废弃` 时 `修订版本号` 保持 `1`、`废弃原因` 记「初稿期作废」。**MUST NOT** 以删除代替作废。标记格式与引用追溯见 [item-deprecation.md](references/item-deprecation.md)。

### 废弃整份文档处理

两阶段：先原地标 `废弃` 并给**建议归档日期**；到期归档入本作用域 `deprecated/`（只归档不删除，各子项目独立，无 `active/`）。流程见 [doc-deprecation.md](references/doc-deprecation.md)。
...
4. **执行升格与提交**：明确确认者 → `正式`（定稿不递增版本号；缺字段补齐）。**升格 MUST 自底向上**：先细项后文档；用户只确认文档而未确认其下细项时 **MUST** 回指待定稿细项。未提及 / 略过 → 保持未冻结。然后按用户指示 `svn commit` / `git commit`（是否 push 另请示）。

**MUST NOT**：未经确认升格；扫描并提交本轮未改的历史未冻结对象；删除已分配编码或已建档文档（放弃走作废，须 Checkpoint 4；`草案` 另有回退，由人决定）。
...
- `--check-templates`：模板哨兵（成对、唯一 H1、无残留外层围栏、使用说明未混入待复制正文）。
- `--instantiate {模板文件名} [--segment {段名}]`：剥除哨兵输出实例化后的正文；多段模板（`ref.md`、`readme-template.md`）用 `--segment` 取单段。
- `python3 scripts/tests/run_fixtures.py`：夹具回归。每条夹具只验证一条规则；跑台只断言目标问题名，忽略夹具极简结构带来的噪声。改 `check_docs.py` 后 **MUST** 跑通；新增校验规则 **MUST** 同时补正反夹具并登记入跑台的期望表。脚本只出静态提示，不代替人工审查与定稿确认。
...
**Q: 可以改已分配编码的标题或含义吗？**  
A: 按 [object-model.md · 修改操作矩阵](references/object-model.md#修改操作矩阵must-comply)。ID / 类型码永不就地改；标题自 `正式` 起锁定；仅改正文走解冻。
