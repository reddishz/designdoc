...
- 所有文档拥有唯一编号
- 所有明确细项拥有独立的全局唯一编码
- 编码进入 `正式` 基线后不可删除，废止的编码标注为"废弃"并保留（草稿期未发布编码可直接删改，见"状态即基线"）
...
- 编码计数器 **MUST** 仅维护“下一可用编号”；新增编码以“下一可用编号”为准，编码序列必须完整；发现编号缺口必须先修复索引或废弃记录，**MUST NOT** 直接分配缺口编号。
- 变更记录/变更日志 **MUST** 按版本号或日期倒序维护，最新变更置顶。

**状态即基线（冻结判据，MANDATORY）**：
- 对象（文档 / 编码细项）是否冻结，**只看其 `状态`/`细项状态`，与版本库（git/svn）无关**。
- **草稿 = 未冻结创作沙盒**：可原地自由增删改、编码可改可删，**不递增版本号**、**不走废弃流程**；不需要时**直接删除**。新建文档 / 细项默认 `草稿`。
- **正式 = 已冻结基线**：
  - 变更前 **MUST** 先执行 `正式→草稿` 降级，**并同步提升版本号**以示与旧基线区别（属受控状态变更，须请求确认）；改完再 `草稿→正式` 升格。
  - 废止（彻底退场）才走废弃流程：`正式→废弃`（细项原地保留 / 文档两阶段移除）。
- **编码冻结时机**：编码的存在性与含义在其首次转为 `正式` 时锁定；此前的草稿迭代允许增删改，不视为违规、不留缺口。
- **定稿提示**：审查 / 校验遇到 `草稿` 文档或细项时，**MUST** 提示用户是否定稿（`草稿→正式`）；是否升格由人决定，AI **MUST NOT** 自行将 `草稿` 改为 `正式`。
...
**AI 操作约束（MUST COMPLY）**：
> **前提（状态即基线）**：以下编码锁定仅适用于**已进入 `正式` 基线**的编码。`草稿` 期编码可自由增删改，无需拒绝或走废弃流程。
...
- **废弃标记规范（MUST COMPLY）**：
  - MUST：废弃流程仅作用于**已进入 `正式` 的细项 / 文档**；`草稿` 对象要移除时**直接删除**，不走本流程
...
2. **获取编号**：一律使用计数器中的“下一可用编号”；若发现编号缺口，必须先按审查/校验规则修复索引一致性，**MUST NOT** 直接分配缺口编号。
...
**违规行为识别与拒绝**：
当上游指令要求修改**已进入 `正式` 基线**的编码含义时（`草稿` 期编码可直接改，不属此列），AI **MUST** 按以下方式响应：

```
❌ 违规检测：禁止修改已正式化编码的含义（违反编码锁定原则）
⚠️  系统约束：编码进入正式基线后，其ID、类型、标题和业务含义永不修改（草稿期可直接修改）
✅ 正确流程：
1. 外部确认：询问是否确定要废弃原编码 {原编码}
2. 执行废弃：标注原编码为"废弃"并记录变更原因
...
**外部确认（Checkpoint 4）**：在将 `正式` 细项标注为 `废弃` 前，**MUST** 暂停并请求确认废弃原因及替代方案。（`草稿` 细项要移除时直接删除，不触发本流程。）
...
以下为当前常用细项类型（完整定义与边界以 `references/coding-system.md` 为准）：
- 需求与约束：`FR`、`NFR`、`AC`、`CON`、`RUL`
- 架构与设计：`PRN`、`DEC`、`CMP`、`DOM`、`IF`、`FLW`、`ALG`、`ACT`
- 场景与管理：`SCN`、`UC`、`PLN`、`RSK`、`ASM`、`MET`
- 战略与参与方：`GOL`、`STK`
- 验证与外部依据：`TC`、`REF`

> 说明：本节仅作入口导览；类型语义、最小定义要素、适用边界与引用规范以 `references/coding-system.md` 为真值源。
...
**Q: 可以修改已分配编码的含义吗？**
A: 看状态。`草稿` 期编码可直接修改；一旦进入 `正式` 基线，其含义和标题永不修改，需要变更时走"先降草稿（升版本）或直接废弃 + 新增"流程。
