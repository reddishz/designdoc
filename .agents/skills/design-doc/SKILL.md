...
AI 在创建或编辑设计文档时，模板中的 `{当前用户.作者}` 与 `{项目编码}` 按以下规则解析。
...
**提示规则（新增强制约束）**：
- 当 AI 识别到 `ued/` 下存在多个应用或多个独立业务域的设计文档时，**MUST** 提示设置 `ued/.doc-config.json` 中的 `project_code`，并建议对后续新增编码启用项目编码前缀。
- 当 AI 识别到当前仓库已存在某一应用的设计文档，而当前任务是在此基础上新增另一应用的设计文档时，**MUST** 在创建前提示设置并启用项目编码，以保证多应用场景下的唯一索引与引用清晰性。
...
随后 AI **SHOULD** 提醒补充或编辑 `ued/.doc-config.json`，特别是当检测到目录中存在多个子项目、多应用或新增独立应用迹象时。
