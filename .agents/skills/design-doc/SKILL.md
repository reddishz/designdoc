...
- **废弃标记规范（MUST COMPLY）**：
  ...
  - SHOULD：尽可能提供替代方案字段
  - WHEN AVAILABLE：废弃人字段仅在可获取时填写（从 `ued/.doc-config.json` 的 `author` 字段获取）
...
**废弃原因**：{具体原因}
**替代方案**：{替代细项编码}（如有）
**废弃人**：{废弃人姓名}（可获取时，从 `ued/.doc-config.json` 的 `author` 字段获取）
```

**字段说明**：
- **必须字段（MUST）**：细项状态、废弃时间、废弃原因
- **可选字段（SHOULD）**：替代方案
- **条件字段（WHEN AVAILABLE）**：废弃人
...
3. 推荐提供替代方案（如有）
4. 废弃人字段仅在可获取时填写（从 `ued/.doc-config.json` 的 `author` 字段获取）
5. 必须更新全局索引和文档清单
6. 必须追溯并处理所有引用关系
